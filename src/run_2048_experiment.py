...
from src.agents import IDExpectiMaxAgent, MaximizerMCTSAgent, RandomDistributionAgent


agent_dict = {agent.__name__: agent for agent in (IDExpectiMaxAgent, MaximizerMCTSAgent)}
safe_float = lambda x: round(float(x), 2) if "," not in x else round(float(x.replace(",", ".")), 2)

# define command line arguments
parser = argparse.ArgumentParser(description='Run a number of 2048 games with \
...
parser.add_argument(
    '-t', help="The time available to the agent for taking an action.", type=safe_float, default=1)
